...
import TerminalAIComponent from "./TerminalAIComponent";
...
const TerminalCenter = () => {
  // AI panel open state — on mobile this becomes the full-screen overlay
  ...
  useEffect(() => {
    ...
    const runAnimation = async () => {
      ...
      controls = animate(0, fullTitle.length, {
        duration: fullTitle.length * 0.035,
        ease: "linear",
        onUpdate: (latest) => {
      ...
      controls = animate(0, fullDesc.length, {
        duration: fullDesc.length * 0.025,
        ease: "linear",
        onUpdate: (latest) => {
      ...
      for (let i = 0; i < DOCUMENT_BLOCKS.length; i++) {
        ...
        controls = animate(0, block.text.length, {
          duration: block.text.length * 0.015,
          ease: "linear",
          onUpdate: (latest) => {
  ...
  return (
    <div className="relative flex w-full bg-background overflow-hidden" style={{ height: "calc(100% - 0px)" }}>
      <style>{`
        @keyframes blink {
          0%, 100% { opacity: 1; }
          50% { opacity: 0; }
      ...
      {/* Editor pane */}
      <section className="px-2 py-2 w-full overflow-y-auto">
        <div className="flex w-full justify-between items-center mb-2">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {/* AI button — on mobile opens the fullscreen overlay */}
      ...
      {/* AI panel — fullscreen overlay on mobile, hidden on md+ (Terminal only shows on mobile) */}
      <TerminalAIComponent isOpen={aiOpen} onClose={() => setAiOpen(false)} />
    </div>
  );
};

export default TerminalCenter;
